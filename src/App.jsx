    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1E1B4B 0%,#312E81 50%,#1E1B4B 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#fff",borderRadius:20,padding:"40px 36px",width:"100%",maxWidth:400,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{fontSize:22,fontWeight:800,color:"#1E1B4B"}}>APT CONTROL PANEL</div>
        ...
        <div style={{textAlign:"center",marginTop:16,fontSize:12,color:"#94A3B8"}}>© Firmandev MewOne - APT CONTROLLER</div>
      </div>
    </div>
  );
}
...
function ProductsPage() {
  ...
  const [editPrice,setEditPrice]=useState(null); // {id, value}
  ...
  const savePrice=async(id)=>{
    const newPrice=parseInt(String(editPrice.value).replace(/\D/g,""));
    if(!newPrice||newPrice<=0)return toast.error("Harga tidak valid!");
    try{
      await api(`/api/products/${id}`,"PUT",{price:newPrice});
  ...
  return (
    <div>
      ...
      {showForm&&<Card style={{marginBottom:20}}>
        <div style={{fontSize:15,fontWeight:600,marginBottom:16}}>Produk Baru</div>
        ...
        {/* Baris 2: Deskripsi full width — textarea bisa resize vertikal */}
        <div style={{marginBottom:12}}>
          <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:5,fontWeight:500}}>Deskripsi</label>
          <textarea
            value={form.description}
            onChange={e=>setForm({...form,description:e.target.value})}
            placeholder="Tulis deskripsi produk di sini... Contoh: Akun Netflix Premium 1 bulan, garansi 30 hari, bisa sharing 1 layar."
            rows={4}
            style={{
        ...
        {/* Baris 3: Checkbox Pre-Order */}
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,padding:"10px 14px",background:"#F8FAFC",borderRadius:8,border:"1px solid #E2E8F0"}}>
          <input type="checkbox" id="po" checked={form.preorder_only} onChange={e=>setForm({...form,preorder_only:e.target.checked})} style={{width:16,height:16,cursor:"pointer",accentColor:"#6366F1"}}/>
          <div>
            <label htmlFor="po" style={{fontSize:13,fontWeight:500,cursor:"pointer",color:"#374151"}}>Pre-Order Only</label>
            <div style={{fontSize:11,color:"#94A3B8",marginTop:1}}>Produk bisa dipesan meski stok kosong</div>
          </div>
        </div>

        <div style={{display:"flex",gap:8}}>
          <Btn onClick={save} color="#10B981" disabled={saving}>{saving?"⏳ Menyimpan...":"💾 Simpan Produk"}</Btn>
        ...
        :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:16}}>
          {products.map(p=>(
            <Card key={p.id} style={{padding:18}}>
              ...
              {p.description&&<div style={{fontSize:12,color:"#94A3B8",marginBottom:8,lineHeight:1.5}}>{p.description}</div>}

              {/* Harga — klik untuk edit inline */}
                ...
                {editPrice?.id===p.id ? (
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    <input
                      ...
                      autoFocus
                      style={{flex:1,padding:"6px 10px",border:"2px solid #6366F1",borderRadius:7,fontSize:14,fontWeight:700,color:"#10B981"}}
                    />
                    <button onClick={()=>savePrice(p.id)} style={{padding:"6px 10px",background:"#10B981",color:"#fff",border:"none",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:700}}>✓</button>
                    <button onClick={()=>setEditPrice(null)} style={{padding:"6px 10px",background:"#F1F5F9",color:"#64748B",border:"none",borderRadius:7,cursor:"pointer",fontSize:12}}>✕</button>
                  </div>
                ) : (
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div
                      ...
                      style={{fontSize:17,fontWeight:700,color:"#10B981",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}
                    >
                      {formatIDR(p.price)}
              ...
              {p.created_by&&<div style={{fontSize:11,color:"#94A3B8",marginBottom:10}}>Dibuat: {p.created_by}</div>}
...
function StockPage() {
  ...
  const [sel,setSel]=useState("");          // produk untuk tambah stok
  const [filterProd,setFilterProd]=useState(""); // filter tampilan
  ...
  const loadStock    = useCallback(() => {
    let url = "/api/stock";
    const params = [];
    if (filterProd)     params.push(`product_id=${filterProd}`);
    ...
    <div>
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #F1F5F9",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <span style={{fontSize:14,fontWeight:600,color:"#0F172A",marginRight:4}}>Filter Produk:</span>
          {/* Filter by product */}
          <select
            value={filterProd}
            onChange={e=>setFilterProd(e.target.value)}
            style={{padding:"5px 10px",borderRadius:8,border:"1px solid #E2E8F0",fontSize:12,background:"#fff",cursor:"pointer",minWidth:140}}
          >
          ...
          <div style={{display:"flex",gap:6,marginLeft:"auto"}}>
            {["available","sold","all"].map(f=>(
              <button key={f} onClick={()=>setStatusFilter(f)} style={{padding:"4px 12px",borderRadius:16,border:"1px solid",fontSize:12,cursor:"pointer",background:statusFilter===f?"#6366F1":"#fff",color:statusFilter===f?"#fff":"#64748B",borderColor:statusFilter===f?"#6366F1":"#E2E8F0"}}>
                {f==="available"?"Tersedia":f==="sold"?"Terjual":"Semua"}
              </button>
            ))}
          </div>
          <span style={{fontSize:12,color:"#94A3B8",width:"100%"}}>
            {filterProd ? `Menampilkan: ${getProd(filterProd)}` : "Semua produk"} · {allStock.length} item
          </span>
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {allStock.length===0?<tr><td colSpan={5} style={{textAlign:"center",padding:40,color:"#94A3B8"}}>Tidak ada stok {filterProd?`untuk ${getProd(filterProd)}`:""}</td></tr>
...
function PakasirCard({toast}) {
  ...
  const checkStatus = async () => {
    ...
    try {
      const r = await api(`/api/pakasir/status?order_id=${testOrd}&amount=${testAmt}`);
      setTestResult(r);
    } catch(e) { toast.error(e.message); }
    setTesting(false);
  };
  ...
  return (
    <Card>
      <div style={{fontSize:15,fontWeight:600,marginBottom:14}}>💳 Pakasir Payment Gateway</div>

      {/* Status aktif/tidak */}
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",borderRadius:10,border:"1px solid",background:info?.active?"#D1FAE5":"#FEF3C7",borderColor:info?.active?"#6EE7B7":"#FCD34D",marginBottom:14}}>
        <div style={{fontSize:20}}>{info?.active?"✅":"⚠️"}</div>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:info?.active?"#065F46":"#92400E"}}>
            {info===null?"Memuat...":info?.active?`Aktif — Project: ${info.project}`:"Tidak Aktif"}
          </div>
          <div style={{fontSize:11,color:info?.active?"#16A34A":"#B45309",marginTop:2}}>
            {info?.active?"Payment otomatis aktif saat user bayar":"Set PAKASIR_PROJECT & PAKASIR_API_KEY di Railway"}
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <div style={{marginBottom:14}}>
            <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:5,fontWeight:500}}>
              Webhook URL — Paste ini di Pakasir Dashboard (Edit Proyek)
            </label>
            <div style={{display:"flex",gap:8}}>
              <input
                readOnly
                value={info.webhook}
                style={{flex:1,padding:"9px 12px",border:"1px solid #E2E8F0",borderRadius:8,fontSize:12,fontFamily:"monospace",background:"#F8FAFC",color:"#374151"}}
              />
              <button
                onClick={()=>{navigator.clipboard.writeText(info.webhook);toast.success("Disalin!");}}
                style={{padding:"9px 14px",background:"#6366F1",color:"#fff",border:"none",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600}}
              >
                Salin
              </button>
            </div>
          ...
          <div style={{borderTop:"1px solid #F1F5F9",paddingTop:14}}>
            <div style={{fontSize:13,fontWeight:600,color:"#374151",marginBottom:10}}>Cek Status Transaksi</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:8}}>
              <div>
                <label style={{fontSize:11,color:"#64748B",display:"block",marginBottom:4}}>Order ID</label>
                <input value={testOrd} onChange={e=>setTestOrd(e.target.value)} placeholder="ORD-XXXXXXXX" style={{width:"100%",padding:"8px 10px",border:"1px solid #E2E8F0",borderRadius:7,fontSize:13,boxSizing:"border-box"}}/>
              </div>
              <div>
                <label style={{fontSize:11,color:"#64748B",display:"block",marginBottom:4}}>Amount (IDR)</label>
                <input value={testAmt} onChange={e=>setTestAmt(e.target.value)} placeholder="50000" style={{width:"100%",padding:"8px 10px",border:"1px solid #E2E8F0",borderRadius:7,fontSize:13,boxSizing:"border-box"}}/>
              </div>
              <div style={{paddingTop:19}}>
                <button onClick={checkStatus} disabled={testing} style={{padding:"8px 14px",background:"#6366F1",color:"#fff",border:"none",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:600}}>
                  {testing?"⏳":"Cek"}
                </button>
              </div>
            </div>
            {testResult?.transaction && (
              <div style={{marginTop:10,padding:"12px 14px",background:"#F8FAFC",borderRadius:8,fontSize:12,lineHeight:1.8}}>
                <div>Status: <strong style={{color:testResult.transaction.status==="completed"?"#10B981":"#F59E0B"}}>{testResult.transaction.status}</strong></div>
                <div>Amount: <strong>{formatIDR(testResult.transaction.amount)}</strong></div>
                {testResult.transaction.payment_method&&<div>Metode: {testResult.transaction.payment_method}</div>}
                {testResult.transaction.completed_at&&<div>Selesai: {new Date(testResult.transaction.completed_at).toLocaleString("id-ID")}</div>}
              </div>
            )}
          </div>
        </>
      ...
      {!info?.active && (
        <div style={{padding:"12px 14px",background:"#EFF6FF",borderRadius:8,fontSize:12,color:"#1D4ED8",lineHeight:1.8}}>
          <strong>Cara setup Pakasir:</strong><br/>
          1. Daftar di <a href="https://pakasir.com" target="_blank" rel="noreferrer" style={{color:"#6366F1"}}>pakasir.com</a><br/>
          2. Buat Proyek → catat <strong>Slug</strong> dan <strong>API Key</strong><br/>
          3. Set di Railway Variables:<br/>
          &nbsp;&nbsp;• <code>PAKASIR_PROJECT</code> = slug proyek<br/>
          &nbsp;&nbsp;• <code>PAKASIR_API_KEY</code> = api key<br/>
          &nbsp;&nbsp;• <code>APP_URL</code> = URL Railway kamu<br/>
          4. Di Pakasir Dashboard → Edit Proyek → isi <strong>Webhook URL</strong>:<br/>
          &nbsp;&nbsp;<code>https://URL-RAILWAY/webhook/pakasir</code>
        </div>
      )}
    </Card>
  );
...
function SettingsPage({currentUser}) {
  ...
  useEffect(()=>{
    ...
    api("/api/bot-info").then(info=>{setBotInfo(info);setCustomBotUrl(info.url);}).catch(console.error);
    testConn();
  },[]);
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gap:18}}>
        ...
        {/* Bot Redirect Card */}
        <Card>
          <div style={{fontSize:15,fontWeight:600,marginBottom:14}}>🤖 Tombol Redirect ke Bot</div>
          {botInfo ? (
            <div>
              <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"#F0FDF4",borderRadius:10,border:"1px solid #BBF7D0",marginBottom:14}}>
                ...
                <div>
                  <div style={{fontSize:13,fontWeight:600,color:"#065F46"}}>@{botInfo.username}</div>
                  <div style={{fontSize:12,color:"#16A34A"}}>{botInfo.url}</div>
                </div>
                <a href={botInfo.url} target="_blank" rel="noreferrer" style={{marginLeft:"auto",padding:"8px 16px",background:"#10B981",color:"#fff",borderRadius:8,textDecoration:"none",fontSize:13,fontWeight:600}}>
                  Buka Bot ↗
                </a>
              </div>
              <div style={{marginBottom:10}}>
                <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:5,fontWeight:500}}>Custom URL / Deep Link (opsional)</label>
                <div style={{display:"flex",gap:8}}>
                  <input
                    value={customBotUrl}
                    onChange={e=>setCustomBotUrl(e.target.value)}
                    placeholder={`https://t.me/${botInfo.username}?start=promo`}
                    style={{flex:1,padding:"9px 12px",border:"1px solid #E2E8F0",borderRadius:8,fontSize:13}}
                  />
                  <a href={customBotUrl||botInfo.url} target="_blank" rel="noreferrer" style={{padding:"9px 18px",background:"#6366F1",color:"#fff",borderRadius:8,textDecoration:"none",fontSize:13,fontWeight:600,whiteSpace:"nowrap"}}>
                    Test Link
                  </a>
                </div>
                <div style={{fontSize:11,color:"#94A3B8",marginTop:5}}>
                  Deep link contoh: <code>https://t.me/{botInfo.username}?start=promo</code> — user langsung diarahkan ke bot dengan parameter start
                </div>
              </div>
              <div style={{padding:"10px 14px",background:"#EFF6FF",borderRadius:8,fontSize:12,color:"#1D4ED8",lineHeight:1.7}}>
                💡 <strong>Cara pakai:</strong> Tempel link di bio, website, atau tombol iklan. User yang klik langsung masuk ke bot kamu.
              </div>
            </div>
          ) : (
            <div style={{color:"#94A3B8",fontSize:13}}>⏳ Memuat info bot...</div>
          )}
        </Card>
...
export default function App() {
  ...
  // collapsed: sidebar kecil (hanya ikon), expanded: sidebar penuh
  ...
  const logout = () => { if(window.confirm("Yakin mau logout?")) { clearAuth(); setUser(null); } };
  const nav    = (id) => { setPage(id); setMobileSidebar(false); };
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:"#F8FAFC",fontFamily:"'Inter',-apple-system,sans-serif"}}>
      ...
      {/* Mobile overlay */}
      {mobileSidebar && <div onClick={()=>setMobileSidebar(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:40}}/>}

      {/* ── SIDEBAR ── */}
      <aside style={{
        width:SW, background:"#1E1B4B", display:"flex", flexDirection:"column",
        position:"fixed", top:0, left:0, height:"100vh", zIndex:50,
        transition:"width 0.25s ease", overflow:"hidden"
      }} className={`sidebar${mobileSidebar?" open":""}`}>

        {/* Logo + Toggle collapse button */}
        <div style={{padding:"18px 14px 14px",borderBottom:"1px solid rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          {!collapsed && <div>
            <div style={{color:"#fff",fontSize:16,fontWeight:800,whiteSpace:"nowrap"}}>APT DASHBOARD</div>
            <div style={{color:"#A5B4FC",fontSize:11,marginTop:1}}>Admin Dashboard</div>
          </div>}
          {collapsed && <div style={{color:"#fff",fontSize:20,margin:"0 auto"}}>🛡️ </div>}
          {/* Toggle button — hanya tampil di desktop */}
          <button
            onClick={()=>setCollapsed(!collapsed)}
            className="collapse-btn"
            title={collapsed?"Perluas sidebar":"Sembunyikan sidebar"}
            style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,cursor:"pointer",color:"#A5B4FC",fontSize:14,padding:"4px 7px",flexShrink:0,lineHeight:1}}
          >
            {collapsed ? "▶" : "◀"}
          </button>
        </div>
        ...
        {/* User info */}
        {!collapsed && (
          <div style={{padding:"10px 12px",background:"rgba(99,102,241,0.2)",margin:"10px 10px 4px",borderRadius:10,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            <div style={{width:30,height:30,borderRadius:"50%",background:"#6366F1",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",flexShrink:0}}>{user.username[0].toUpperCase()}</div>
            <div style={{overflow:"hidden"}}>
              <div style={{fontSize:13,fontWeight:600,color:"#fff",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{user.username}</div>
        ...
        {collapsed && (
          <div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px",flexShrink:0}}>
            <div title={user.username} style={{width:34,height:34,borderRadius:"50%",background:"#6366F1",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff",cursor:"default"}}>{user.username[0].toUpperCase()}</div>
          </div>
        )}
          ...
          {visibleNav.map(item=>(
            <button
              ...
              style={{
                ...
                color:page===item.id?"#C7D2FE":"#94A3B8",
                fontSize:14, fontWeight:page===item.id?600:400,
                textAlign:"left",
                borderLeft:page===item.id&&!collapsed?"3px solid #6366F1":"3px solid transparent",
                transition:"all 0.15s", whiteSpace:"nowrap"
              }}
            >
              <span style={{fontSize:collapsed?20:16,flexShrink:0}}>{item.icon}</span>
      ...
      {/* ── MAIN CONTENT ── */}
      <div style={{flex:1, marginLeft:SW, display:"flex", flexDirection:"column", minHeight:"100vh", transition:"margin-left 0.25s ease"}} className="main-content">
        ...
        <header style={{background:"#fff",borderBottom:"1px solid #F1F5F9",padding:"13px 24px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:30}}>
          {/* Mobile hamburger */}
          <button onClick={()=>setMobileSidebar(!mobileSidebar)} className="hamburger" style={{display:"none",background:"none",border:"none",cursor:"pointer",fontSize:22,padding:2,color:"#374151"}}>☰</button>
          <div style={{fontSize:14,fontWeight:600,color:"#64748B"}}>{curNav?.icon} {curNav?.label}</div>
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:12}}>
            <div style={{fontSize:12,color:"#94A3B8"}}>{new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long"})}</div>
            {/* Logout shortcut di topbar (backup kalau sidebar collapsed di mobile) */}
            <button onClick={logout} style={{padding:"5px 12px",background:"#FEE2E2",color:"#DC2626",border:"none",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:600}} className="topbar-logout">
              🚪 Logout
            </button>
          </div>
        </header>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* Desktop: tombol logout di topbar disembunyikan, pakai yang di sidebar */
        ...
        @media (max-width: 768px) {
          ...
          /* Di mobile, tombol logout di topbar muncul sebagai backup */
